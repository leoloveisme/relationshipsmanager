...
class ContactsAdapter(
    ...
    var urgencyMap: Map<Int, UrgencyLevel> = emptyMap()
        private set
    ...
    fun loadUrgencyData(contacts: List<Contact>) {
        ensureBackgroundThread {
            val calculator = UrgencyCalculator(activity)
    ...
    private fun setupView(view: View, contact: Contact, holder: ViewHolder) {
        view.apply {
            ...
            val frame = findViewById<ConstraintLayout>(org.fossify.commons.R.id.item_contact_frame)
            frame?.isSelected = selectedKeys.contains(contact.id)

            // Inject urgency indicator bar
            if (frame != null) {
                val urgencyLevel = urgencyMap[contact.id] ?: UrgencyLevel.NONE
                var indicator = frame.findViewWithTag<View>("urgency_indicator")
                if (urgencyLevel != UrgencyLevel.NONE) {
                    if (indicator == null) {
                        indicator = View(context).apply {
                            tag = "urgency_indicator"
                            id = View.generateViewId()
                            layoutParams = ConstraintLayout.LayoutParams(
                                resources.getDimensionPixelSize(R.dimen.urgency_indicator_width),
                        ...
                        frame.addView(indicator, 0)
                    }
                    indicator.setBackgroundColor(ContextCompat.getColor(context, urgencyLevel.colorResId))

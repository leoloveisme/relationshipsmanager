...
class ViewContactActivity : ContactActivity() {
    ...
    private fun setupUrgencyStatus() {
        val currentContact = contact ?: return
        ensureBackgroundThread {
            val calculator = UrgencyCalculator(this)
            val urgencyLevel = calculator.calculateUrgency(currentContact)
            val daysSince = calculator.getDaysSinceLastInteraction(currentContact.id)
            runOnUiThread {
                if (isDestroyed || isFinishing) return@runOnUiThread

                if (urgencyLevel == UrgencyLevel.NONE) {
                    binding.contactUrgencyImage.beGone()
                    binding.contactUrgencyHolder.beGone()
                    return@runOnUiThread
                }

                binding.contactUrgencyImage.beVisible()
                binding.contactUrgencyHolder.beVisible()
                binding.contactUrgencyImage.applyColorFilter(getProperTextColor())

                val colorRes = urgencyLevel.colorResId
                val color = ContextCompat.getColor(this, colorRes)
                (binding.contactUrgencyDot.background.mutate() as? GradientDrawable)?.setColor(color)

                binding.contactUrgencyLabel.text = when (urgencyLevel) {
                    UrgencyLevel.GREEN -> getString(R.string.urgency_green_label)
                    UrgencyLevel.YELLOW -> getString(R.string.urgency_yellow_label)
                    UrgencyLevel.ORANGE -> getString(R.string.urgency_orange_label)
                    UrgencyLevel.RED -> getString(R.string.urgency_red_label)
                    UrgencyLevel.NONE -> ""
                }
                binding.contactUrgencyLabel.setTextColor(getProperTextColor())

                if (daysSince != null) {
                    binding.contactUrgencyDays.text = getString(R.string.days_since_contact, daysSince.toInt())
                } else {
                    binding.contactUrgencyDays.text = getString(R.string.no_interaction_history)
                }
                binding.contactUrgencyDays.setTextColor(getProperTextColor())

                binding.contactMarkContacted.setOnClickListener {
                    ensureBackgroundThread {
                        relationshipRepository.insertInteraction(
                            ContactInteraction(
                                contactId = currentContact.id,
                                interactionType = INTERACTION_MANUAL,
